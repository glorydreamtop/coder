<template>
	<view class="all">
		...
		<view class="boxes bg-white flex justify-between align-center">
			...
			<view class="btns">
				<button :class="['cu-btn','round',item.selected ? `bg-${item.color}` : `line-${item.color}`,'shadow','margin-right-xs','sm']"
				 @tap="checkPic(item,index)" v-for="(item,index) in pics" :key="item.name">{{item.name}}</button>
			</view>
		</view>
		<view class="boxes bg-white text-red text-bold text-lg text-center" @tap="clearStorge">
			清除所有缓存
		</view>
	</view>
</template>
