<template>
	<view class="content">
		<view class="tagList flex justify-start text-white">
			<view :class="currentsearchType === item.type ? 'text-blue' : ''" v-for="item in searchTypes" :key="item.type" @tap="selectType(item.type)">{{ item.title }}</view>
		</view>
		<view class="resList" v-show="currentsearchType === searchTypes[0].type">
			<mescroll-item class="me" ref="ARTICLE" top="80" :mescrollOption="mescrollOption" :dataList="dataList[0]" @up="update" />
		</view>
		<view class="resList" v-show="currentsearchType === searchTypes[1].type"><user-item ref="USER" class="me" :mescrollOption="mescrollOption" :dataList="dataList[1]" @up="update" /></view>
	</view>
</template>

<script>
import { search,follow } from '../../utils/request';
